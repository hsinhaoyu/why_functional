...
=foldtree= doesn't do any yielding because it doesn't necessarily return a lazy structure. If we want it to return a lazy structure, =f= and =g= need to yield. Here's a function that visits labels in a tree one by one, and returns them as an iterator:
#+begin_src python :noweb yes :tangle ../src/lazy_utils.py
